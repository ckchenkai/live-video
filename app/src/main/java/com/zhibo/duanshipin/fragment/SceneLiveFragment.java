...
import com.zhibo.duanshipin.widget.LoadingDialog;
 ...
 * 视频播放
 */
public class SceneLiveFragment extends BaseLazyFragment implements View.OnClickListener {
    private static final String TAG = SceneLiveFragment.class.getSimpleName();
...
//        mActivity.getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
    ...
    private void initData() {
        ...
        if(fragmentList.size()>0){ //刷新
            if (termId != null) {
                mLiveListFragment.refreshData(termId);
            }
            mIntroductionFragment.refreshData(description);
        }else{                   //初始化
            if (termId != null) {
                titleList.add("图文直播");
            ...
            titleList.add("简介");
            mIntroductionFragment = IntroductionFragment.newInstance(description);
            fragmentList.add(mIntroductionFragment);
            ...
            videoPlayer.setJzUserAction(new JZUserAction() {
                ...
                public void onEvent(int type, Object url, int screen, Object... objects) {
                    if (type == JZUserAction.ON_AUTO_COMPLETE) {
                        ...
                        loadingDialog = new LoadingDialog(mActivity,R.style.MyDialogStyle);
                        loadingDialog.show();
