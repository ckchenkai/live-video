...
public class LRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public int getSpanSize(int position,int spanSize){
        boolean isHeaderOrFooter =isRefreshHeader(position) ||isHeader(position) || isFooter(position);
    ...
    /**
     * 获取数据存放的实际位置（除去刷新头部的位置和普通头部位置）
     * 刷新除去头部的所有数据
     */
    /*public void notifyDataExpHead(){
        notifyItemRangeChanged(mHeaderViews.size()+1,getInnerAdapter().getItemCount());
    }*/
}
